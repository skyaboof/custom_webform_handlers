        ...
        once('distance-calculator', 'input[name="origin_address"], input[name="destination_address"]', context).forEach((element) => {
          ...
          async function calculateDistance() {
            if (fromInput.value.trim() && toInput.value.trim()) {
              const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(fromInput.value.trim())}&destinations=${encodeURIComponent(toInput.value.trim())}&mode=driving&units=metric&key=${apiKey}`;
              ...
              try {
                ...
                if (data.status === 'OK' && data.rows[0].elements[0].status === 'OK') {
                  ...
                  console.warn('DistanceMatrix result not OK', data.status, data.rows[0].elements[0].status);
